...
type AuditCase = {
  ...
  sources: { label: string; detail: string; url: string }[]; factBased?: boolean;
};
...
export default function Home() {
  ...
  return (
    <main className="game-shell" style={{ "--case-accent": activeCase.accent } as CSSProperties}>
      ...
      <div className="notice-strip"><b>공시 기반 문제</b><span>＋</span><b>가상 재구성 사건</b><p>두 유형을 사건별로 표시합니다. 가상 재구성은 해당 기업에서 실제 발생한 사실을 뜻하지 않습니다.</p><button onClick={() => setNoticeOpen(true)}>전체 안내 보기</button></div>
        ...
        <>
          ...
          <section className="case-grid">{cases.map((item) => (
            <button className="case-tile" key={item.id} onClick={() => startCase(item.id)} style={{ "--tile-accent": item.accent } as CSSProperties}>
              <div className="tile-top"><span>CASE {item.number} · {item.factBased ? "공시 기반" : "가상 재구성"}</span><i>{completedIds.includes(item.id) ? "완료 ✓" : "OPEN →"}</i></div>
          ...
          {screen === "case" ? (
            <section className="workspace" aria-live="polite">
              ...
              <article className="question-card">
                ...
                <div className="scenario-disclosure"><b>{activeCase.factBased ? "공시 기반 감사문제" : "교육용 가상 감사상황"}</b><span>{activeCase.factBased ? "문제의 사실관계와 정답은 공개 감사보고서의 위험·수행절차·결론을 요약했습니다. 오답 선택지는 교육용입니다." : "아래 상황과 선택지는 감사 훈련을 위해 재구성했습니다. 해당 기업에서 실제로 발생한 사실이나 부정행위를 의미하지 않습니다."}</span></div>
      ...
      {noticeOpen && <div className="modal-backdrop notice-backdrop" role="presentation" onMouseDown={closeNotice}><section className="source-modal legal-modal" role="dialog" aria-modal="true" aria-labelledby="notice-title" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={closeNotice} aria-label="닫기">×</button><p className="section-label">BEFORE YOU PLAY</p><h2 id="notice-title">이용 전 꼭 알아두세요.</h2><div className="legal-intro">공개 공시를 감사교육용 게임으로 바꾼 독립 프로젝트입니다. 실제 사실과 가상 상황을 아래 기준으로 구분합니다.</div><div className="legal-list">
        ...
        <article><span>02</span><div><b>사건 유형과 사실 구분</b><p>각 사건은 ‘공시 기반’ 또는 ‘가상 재구성’으로 표시합니다. 공시 기반 사건의 사실관계와 정답은 공개 보고서의 핵심위험·수행절차·결론을 요약하며 오답만 교육용입니다. 가상 재구성 사건의 구체적 상황은 실제 사건이나 부정행위를 의미하지 않습니다.</p></div></article>
